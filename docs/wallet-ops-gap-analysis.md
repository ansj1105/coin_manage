...
- 현재 risk score는 내부 출금 이력 기반까지만 확장되어 있다.
- 주소 blacklist/whitelist 정책 외의 외부 인텔리전스 연동이 없다.
- 신규 기기, 반복 실패 주소, 비정상 패턴은 일부 반영됐지만 장기 패턴/주소 그래프 분석은 없다.
...
- 최근 출금 burst, 신규 목적지, 반복 실패 목적지 같은 행위 기반 신호 확대
